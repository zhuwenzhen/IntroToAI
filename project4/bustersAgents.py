...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    # 1st: find the most likely position of each remaining / uncaptured ghost
    ...
    for i in range(len(livingGhostPositionDistributions)):
        ...
        for pos, prob in beliefs.items():
            if prob > maxProb:
                maxProbPos = pos
                maxProb = prob
        possiblePositionList.append(maxProbPos)

    # 2nd: choose an action that minimizes the distance to the closest ghost

    minDistance = 10e6

    for pos in possiblePositionList:
    ...
    for action in legal:
        ...
        distance = self.distancer.getDistance(successorPosition, minDistPos)
