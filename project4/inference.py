...
class ExactInference(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    # print 'noisyDistance', observation
    ...
    for p in self.legalPositions:
      ...
      if emissionModel[trueDistance] > 0:
        # P(X_t | e_1 ... e_t) = P(x_t | e1 ... e_t-1) * P(e_t | x_t)
  ...
  def elapseTime(self, gameState):
    ...
    # oldPos: ghost's old position
    for oldPos, oldPosProb in self.beliefs.items():
      # print 'oldPos = ' + str(oldPos)
      # print 'old position prob = ' + str(oldPosProb) + '\n'
      if oldPosProb == 0:
        continue
      prevGameState = self.setGhostPosition(gameState, oldPos)
      # what positions the ghost will move to from the provided gameState
      newPosDist = self.getPositionDistribution(prevGameState)
      for newPos, newPosProb in newPosDist.items():
        # print 'new Pos = ' + str(newPos)
        # print 'new position prob = ' + str(newPosProb) + '\n'
        # newPostDist[p] = Pr( ghost is at position p at time t + 1 | ghost is at position oldPos at time t )
        # Elapse time compute  P(x_t|e_1:t-1) = sum P(x_t-1|e_1:t-1) * P(x_t | x_t-1)
        allPossible[newPos] += oldPosProb * newPosProb
...
class ParticleFilter(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    for pos, prob in self.beliefs.items():
        ...
        allPossible[pos] = prob * emissionModel[trueDistance]
        probSum += emissionModel[trueDistance] * prob
    if probSum == 0:
        self.initializeUniformly(gameState)
    else:
        allPossible.normalize()
  ...
  def elapseTime(self, gameState):
    ...
    for i in range(self.numParticles):
        ...
        ghostPositionList.append(pos)

    for pos in legal:
        self.beliefs[pos] = 0
    for pos in ghostPositionList:
        prevState = self.setGhostPosition(gameState, pos)
        newPosDist = self.getPositionDistribution(prevState)
        self.beliefs[util.sample(newPosDist)] += 1
    self.beliefs.normalize()
...
class JointParticleFilter:
  ...
  def initializeParticles(self):
    ...
    "*** YOUR CODE HERE ***"
    import random
    legalPos = random.choice(self.legalPositions)
    # list containing  numParticles tuples, each tuple contains number of ghostPositions
    tupleLen = self.numGhosts
    particleLen = self.numParticles
    # print 'tuple length = ', tupleLen
    # print 'particle length = ', particleLen
    self.particles = [tuple(random.choice(legalPos) for i in xrange(tupleLen)) for j in xrange(particleLen)]
