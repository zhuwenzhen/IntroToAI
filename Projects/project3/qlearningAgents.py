...
class QLearningAgent(ReinforcementAgent):
  ...
  def getQValue(self, state, action):
    ...
    """
    "*** YOUR CODE HERE ***"
    stateList = self.values
    #print("actions",  actions)
    #print(stateList)
    if (state, action) in stateList:
  ...
  def update(self, state, action, nextState, reward):
    ...
    estimate = (1 - self.alpha) * oldEstimate + self.alpha * sample

    self.values[(state, action)] = estimate
...
class ApproximateQAgent(PacmanQAgent):
  ...
  def getQValue(self, state, action):
    ...
    for i in features.keys():
      Qvalue += (self.weight[i] * features[i])
    return Qvalue
  ...
  def update(self, state, action, nextState, reward):
    ...
    for i in f.keys():
      self.weight[i] += self.alpha * correction * f[i]
